            ...
            for k in keys:
                ...
                if isinstance(v, str):
                    ...
                    if t == 'one':
                        ...
                        fks.append({'fk_name': k, 'mfk_name': mfkn,
                                   'results': fkns, 'map_table_name': mtn})
            sql = sql[:-2] + ')'
            print(sql)
            await c.execute(sql)
            r = await get_ids(c, table_name, unique_fields, pkey_name, result)
            for i in fks:
                sql = 'INSERT INTO "' + i.get('map_table_name') + '" ("' + i.get(
                    'fk_name') + '", "' + i.get('mfk_name') + '") VALUES ('
